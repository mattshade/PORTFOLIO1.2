...
export function SystemBoids({
  ...
  useEffect(() => {
    ...
    // Performance optimization: Cache wire rects and update them less frequently
    let cachedWires: any[] = []
    const updateWireCache = () => {
      const wires = document.querySelectorAll('.bird-wire, .bird-perch, .bird-perch-card, .nav')
      const sy = window.scrollY
      cachedWires = Array.from(wires).map((w) => {
        ...
        return {
          el: w,
          pageTop: rect.top + sy,
          pageBottom: rect.bottom + sy,
          pageLeft: rect.left,
          width: rect.width,
          height: rect.height,
          isNav, isPerch, isCard,
          isNavPerch: isPerch && !!w.closest('.nav')
        }
      })
    }
    updateWireCache()
    ...
    const render = () => {
      try {
        ...
        // Refresh cache significantly less often since we use page-relative coords
        if (frame % 120 === 0) {
          updateWireCache()
        }
        ...
        // Calculate current viewport rects without layout thrashing
        const activeWires = cachedWires
          .map((c) => {
            const isHovered = c.el.matches(':hover')
            if (isHovered) {
              const wasHovered = hoverStates.get(c.el) || false
              if (!wasHovered) hoverStates.set(c.el, true)
            } else {
              hoverStates.set(c.el, false)
            }

            let top = c.pageTop - sy
            let bottom = c.pageBottom - sy
            
            // Handle fixed nav correctly
            if (c.isNav) {
              const navRect = c.el.getBoundingClientRect()
              top = navRect.top
            ...
            return {
              ...c,
              top: c.isNav ? bottom : top, // Nav landing is at the bottom
              bottom,
              left: c.pageLeft,
              justHovered: isHovered && !(hoverStates.get(c.el) || false)
            }
          })
          .filter((r) => r.bottom > -100 && r.top < height + 100)

        // React to scrolling — high flight sensitivity
        const scrollDelta = Math.abs(currentScroll - lastGlobalScroll)
        if (scrollDelta > 2) {
          const flightProb = Math.min(0.2, scrollDelta * 0.05)
          for (const boid of flock) {
            if (boid.isLanded && Math.random() < flightProb) {
              boid.isLanded = false
        ...
        if (activeWires.length > 0) {
          for (const b of flock) {
            if (b.isLanded) {
              const nearest = activeWires.find(w => w.el === b.landedElement) || activeWires[0]
              
              if (nearest.el.matches(':hover')) {
                b.isLanded = false
              ...
              const yOffsetNearest = nearest.isPerch ? nearest.height * 0.35 : -2
              b.landY = nearest.top + yOffsetNearest
              b.landX = b.landX // Keep same relative X
              
              if (idleTime > 5000 && Math.random() < 0.002 && b.hopTimer === 0) {
                b.hopTimer = 30
              }
            } else {
              ...
              const boidsWires = activeWires.filter(w => {
                ...
                // Prevent even targeting a hovered wire
                if (w.el.matches(':hover')) return false
                return true
              })
              
              if (boidsWires.length === 0) {
              ...
              const yOffset = wire.isPerch ? wire.height * 0.35 : -2
              ...
              // Standard landing arrival logic
              if (d < 2) {
                b.isLanded = true
                ...
                const steer = 0.5 * Math.min(1, 120 / d)
                b.vx += (dx / d) * steer
                b.vy += (dy / d) * steer
        ...
        const visibleRatio = Math.max(0.05, 1 - sy / 3000)
        ...
        console.error('SystemBoids render error:', err)
      }
